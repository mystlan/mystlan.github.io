(()=>{
  ...
  const hasMeaningfulLocalData=()=>{
    const app=safeParse(APP_KEY,{}),vocab=safeParse(VOCAB_KEY,{}),frag=safeParse(FRAG_KEY,[]),target=localStorage.getItem(TARGET_KEY)||'';
    return Boolean(Object.keys(app.daily||{}).length||Object.keys(app.taskStatus||{}).length||Object.keys(vocab).length||frag.length||target);
  };
  const snapshot=()=>({
  ...
  function buildAuthPanel(){
    ...
    panel.innerHTML=`
      <div class="section-head">
        <div><h2>☁️ 云端同步</h2><p>登录后，任务、墨墨、碎片池、目标日期和历史记录会同步到 Supabase。</p></div>
      ...
      <div id="cloudLoggedIn" style="display:none;margin-top:14px">
        <div style="display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap"><button id="cloudSyncNow" type="button">立即同步</button><button id="cloudSignOut" type="button">退出登录</button></div>
        </div>
      </div>
  ...
  function updateAuthUI(user){
    ...
    if(user){
      ...
      const last=localStorage.getItem(LAST_SYNC_KEY);setCloudStatus(last?`上次同步：${humanTime(last)}`:'已登录，等待首次同步');
    }else{
  ...
  async function pushSnapshot(manual=false){
    ...
    const now=new Date().toISOString();originalSetItem.call(localStorage,LAST_SYNC_KEY,now);setCloudStatus(`已同步：${humanTime(now)}`,'ok');
  }
  function scheduleSync(){
    if(!currentUser||suppressSync)return;
    clearTimeout(syncTimer);
    syncTimer=setTimeout(()=>pushSnapshot(false),650);
  }
  ...
  async function hydrateFromCloud(user){
    ...
    if(syncedUser!==user.id){
      ...
      try{
        ...
        if(data.target_date)originalSetItem.call(localStorage,TARGET_KEY,data.target_date);else localStorage.removeItem(TARGET_KEY);
        originalSetItem.call(localStorage,SYNC_USER_KEY,user.id);
        originalSetItem.call(localStorage,LAST_SYNC_KEY,data.updated_at||new Date().toISOString());
      }finally{suppressSync=false}
      setCloudStatus('已从云端恢复数据，正在刷新…','ok');
  ...
  async function init(){
    ...
    client.auth.onAuthStateChange(async(event,sessionNow)=>{
      ...
      currentUser=next;
      updateAuthUI(next);
      if(next&&['SIGNED_IN','TOKEN_REFRESHED','INITIAL_SESSION'].includes(event))await hydrateFromCloud(next);
    });
  }
