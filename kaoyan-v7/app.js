...
const state=JSON.parse(localStorage.getItem(APP_KEY)||'{"taskStatus":{},"customTasks":[],"daily":{},"chapter":{},"shift":"late","generatedTasks":[]}');
state.taskStatus=state.taskStatus||{};state.customTasks=state.customTasks||[];state.daily=state.daily||{};state.chapter=state.chapter||{};state.shift=state.shift||'late';state.generatedTasks=state.generatedTasks||[];
...
async function boot(){
  ...
  renderCountdown();
  if(!state.generatedTasks.length)generatePlan(false);
...
function bind(){
  ...
  $('#shiftSelect').addEventListener('change',()=>{state.shift=$('#shiftSelect').value;save();renderPlanner()});
  $('#generatePlan').addEventListener('click',()=>generatePlan(true));
}
function renderCountdown(){
  const [y,m,d]=planConfig.targetDate.split('-').map(Number),target=Date.UTC(y,m-1,d),now=new Date(),today=Date.UTC(now.getFullYear(),now.getMonth(),now.getDate());
  const days=Math.max(0,Math.ceil((target-today)/86400000));
  $('#countdownDays').textContent=days;$('#targetDateLabel').textContent=`目标日 ${planConfig.targetDate}`;
}
...
function allTasks(){return [...plannerTasks(),...state.customTasks]}
function renderPlanner(){
  const p=planConfig.profiles[state.shift]||planConfig.profiles.late;
  $('#shiftSelect').value=state.shift;$('#planTarget').textContent=`建议有效学习 ${p.targetHours}`;
  $('#planAdvice').innerHTML=`<b>${p.label}</b><span>${p.note}</span>`;
  $('#planTimeline').innerHTML=p.slots.map((s,i)=>`<div class="timeline-row"><div class="timeline-time">${s.time}</div><div class="timeline-dot"></div><div class="timeline-content"><b>${s.subject}｜${s.title}</b><span>建议 ${s.minutes} 分钟</span></div></div>`).join('');
}
function generatePlan(resetStatuses=true){
  ...
  state.generatedTasks=p.slots.map((s,i)=>({id:`${date}-${state.shift}-${i+1}`,subject:s.subject,title:s.title,minutes:s.minutes,time:s.time,generated:true}));
...
function deleteTask(t){state.customTasks=state.customTasks.filter(x=>x.id!==t.id);state.generatedTasks=state.generatedTasks.filter(x=>x.id!==t.id);state.taskStatus[t.id]='delay';save();renderToday()}
